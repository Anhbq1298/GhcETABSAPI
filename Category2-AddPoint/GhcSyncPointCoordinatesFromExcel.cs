...
//   3) sheetName  (string, item)  Worksheet name. Defaults to "Point Coordinates".
//   4) baseline   (generic, tree) OPTIONAL. Values tree from GhcGetPointInfo for comparison/highlighting.
//
// Outputs:
//   0) values     (generic, tree) Column-wise branches (4) read from Excel (header row excluded).
//   1) actions    (string, list)  Rename/move actions executed on ETABS points.
//   2) messages   (string, list)  Summary + diagnostics.
//
// Behavior Notes:
//   • Reads columns A..D (4 columns) from the specified sheet; row 1 treated as headers.
...
//   • When X/Y/Z differ from the current coordinates, the point is selected and EditGeneral.Move drives the translation.
...
namespace MGT
{
    ...
    {
        private const string DefaultSheetName = "Point Coordinates";
        ...
        {
            p.AddGenericParameter("values", "values", "Column-wise data tree (4 branches) read from Excel (header row excluded).", GH_ParamAccess.tree);
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < totalRows; i++)
                    {
                        ...
                        string baselineName = baselineData.GetUniqueName(i);
                        ...
                        {
                            messages.Add($"{rowLabel}: Unable to retrieve current coordinates for '{workingName}'.");
                            UiHelpers.UpdateAssignmentProgressBar(++processed, totalRows, BuildProgressStatus(processed, totalRows));
                            continue;
                        }

                        double targetX = excelData.GetCoordinate(excelData.X, i) ?? currentX;
                        double targetY = excelData.GetCoordinate(excelData.Y, i) ?? currentY;
                        double targetZ = excelData.GetCoordinate(excelData.Z, i) ?? currentZ;

                        double deltaX = targetX - currentX;
                        double deltaY = targetY - currentY;
                        double deltaZ = targetZ - currentZ;

                        if (HasSignificantDelta(deltaX, deltaY, deltaZ))
                        {
                            bool selectionApplied = false;
                            try
                            {
                                if (!TrySetPointSelected(sapModel, workingName, true, out int selectRet))
                                {
                                    string detail = selectRet >= 0 ? $" (code {selectRet})" : string.Empty;
                                    messages.Add($"{rowLabel}: Unable to select '{workingName}' for moving{detail}. Skipped translation.");
                                }
                                else if (!TryMoveSelection(sapModel, deltaX, deltaY, deltaZ, out int moveRet))
                                {
                                    selectionApplied = true;
                                    string detail = moveRet >= 0 ? $" (code {moveRet})" : string.Empty;
                                    messages.Add($"{rowLabel}: Move failed for '{workingName}'{detail}.");
                                }
                                else
                                {
                                    selectionApplied = true;
                                    moveCount++;
                                    actions.Add($"{rowLabel}: Moved '{workingName}' by ({deltaX:0.###}, {deltaY:0.###}, {deltaZ:0.###}) to ({targetX:0.###}, {targetY:0.###}, {targetZ:0.###}).");
                                }
                            }
                            finally
                            {
                                if (selectionApplied)
                                {
                                    TrySetPointSelected(sapModel, workingName, false, out _);
                                }
                            }
                        }
                    ...
                    if (moveCount > 0)
                    {
                        messages.Add($"Updated coordinates for {moveCount} point(s).");
                    }
                }
        ...
        private static void TryClearSelection(cSapModel model)
        {
            if (model == null)
            {
                return;
            ...
            try
            {
                object selectObj = model.SelectObj;
                if (selectObj == null)
                {
                    return;
                }

                Type type = selectObj.GetType();
                MethodInfo method = type.GetMethod("ClearSelection", Type.EmptyTypes);
                if (method != null)
                {
                    method.Invoke(selectObj, Array.Empty<object>());
                }
            }
            catch
            {
        ...
        private static bool TrySetPointSelected(cSapModel model, string pointName, bool selected, out int retCode)
        {
            retCode = -1;
            if (model == null || string.IsNullOrWhiteSpace(pointName))
            {
            ...
            object pointObj = model.PointObj;
            if (pointObj == null)
            {
                return false;
            }
            ...
            try
            {
                ...
                MethodInfo direct = type.GetMethod("SetSelected", new[] { typeof(string), typeof(bool) });
                if (direct != null)
                {
                    object result = direct.Invoke(pointObj, new object[] { pointName, selected });
                    if (result is int ret)
                    {
                        retCode = ret;
                        return ret == 0;
                    }
                ...
                {
                    if (!string.Equals(method.Name, "SetSelected", StringComparison.Ordinal))
                    {
                        continue;
                    }
                    ...
                    object result = method.Invoke(pointObj, args);
                    if (result is int retAlt)
                    {
                        retCode = retAlt;
                        return retAlt == 0;
                    }
        ...
        private static bool TryMoveSelection(cSapModel model, double dx, double dy, double dz, out int retCode)
        {
            retCode = -1;
            if (model == null)
            {
            ...
            try
            {
                object editGeneral = model.EditGeneral;
                if (editGeneral == null)
                {
                    return false;
                }

                Type type = editGeneral.GetType();
                MethodInfo exact = type.GetMethod(
                    "Move",
                    new[] { typeof(bool), typeof(double), typeof(double), typeof(double) });
                if (exact != null)
                {
                    object result = exact.Invoke(editGeneral, new object[] { false, dx, dy, dz });
                    if (result is int retExact)
                    {
                        retCode = retExact;
                        return retExact == 0;
                    }
                ...
                foreach (MethodInfo method in type.GetMethods())
                {
                    if (!string.Equals(method.Name, "Move", StringComparison.Ordinal))
                    {
                        continue;
                    }
                    ...
                    object result = method.Invoke(editGeneral, args);
                    if (result is int ret)
                    {
                        retCode = ret;
                        return ret == 0;
        ...
        private static PointBaselineData ParseBaselineTree(GH_Structure<IGH_Goo> tree)
        {
            ...
            data.X = ExtractDoubleBranch(tree, 1);
            data.Y = ExtractDoubleBranch(tree, 2);
            data.Z = ExtractDoubleBranch(tree, 3);
            return data;
        }
            ...
            Action<int, int, string> progressCallback)
        {
            ...
            try
            {
                ...
                for (int col = 0; col < ColumnCount; col++)
                {
                    ...
                    try
                    {
                        headerCell = ws.Cells[1, ExcelStartColumn + col];
                        object headerValue = headerCell?.Value2;
                        string headerText = TrimOrEmpty(headerValue);
                        data.Headers.Add(headerText);

                        if (!string.Equals(headerText, expectedHeaders[col], StringComparison.OrdinalIgnoreCase))
                        {
                            throw new InvalidOperationException($"Expected header '{expectedHeaders[col]}' in column {(char)('A' + col)}, found '{headerText}'.");
                        }
                    }
                    finally
                    {
                ...
                if (rowCount < 2)
                {
                    progressCallback?.Invoke(0, 0, "Excel sheet empty");
                    return data;
                }

                progressCallback?.Invoke(0, rowCount - 1, BuildExcelProgressStatus(0, rowCount - 1));

                for (int row = 2; row <= rowCount; row++)
                {
                    int processed = row - 2;
                    progressCallback?.Invoke(processed, rowCount - 1, BuildExcelProgressStatus(processed, rowCount - 1));
                    ...
                    for (int col = 0; col < ColumnCount; col++)
                    {
                        ...
                        try
                        {
                            cell = ws.Cells[row, ExcelStartColumn + col];
                            object value = cell?.Value2;

                            bool changed = IsCellChanged(baseline, col, row - 2, value);
                            ...
                            {
                                case 0:
                                    uniqueName = TrimOrEmpty(value);
                                    break;
                                case 1:
                                    x = ParseNullableDouble(value);
                                    break;
                                case 2:
                                    y = ParseNullableDouble(value);
                                    break;
                                case 3:
                                    z = ParseNullableDouble(value);
                                    break;
                            }
                        }
                        finally
                    ...
                    if (string.IsNullOrWhiteSpace(uniqueName) && x == null && y == null && z == null)
                    {
                        continue;
                    }

                    data.UniqueNames.Add(uniqueName);
                    data.X.Add(x);
                    data.Y.Add(y);
                    data.Z.Add(z);
                }

                progressCallback?.Invoke(rowCount - 1, rowCount - 1, BuildExcelDoneStatus(rowCount - 1));
        ...
        private static Excel.Worksheet FindWorksheet(Excel.Workbook workbook, string sheetName)
        {
            ...
            try
            {
                ...
                foreach (Excel.Worksheet sheet in sheets)
                {
                    try
                    {
                        if (sheet != null && string.Equals(sheet.Name, sheetName, StringComparison.OrdinalIgnoreCase))
                        {
                            return sheet;
                        }
                    }
        ...
        private static bool IsCellChanged(PointBaselineData baseline, int columnIndex, int rowIndex, object excelValue)
        {
            ...
            if (columnIndex == 0)
            {
                string baselineValue = baseline.GetUniqueName(rowIndex) ?? string.Empty;
                string excelText = TrimOrEmpty(excelValue);
                return !string.Equals(baselineValue, excelText, StringComparison.Ordinal);
            }

            double? baselineDouble = baseline.GetCoordinate(columnIndex - 1, rowIndex);
        ...
        private static bool IsNullOrEmptyExcel(object value)
        {
            ...
            {
                return Math.Abs(d) < CoordinateTolerance;
            }
        ...
        private static GH_Structure<GH_ObjectWrapper> BuildValueTree(ExcelPointData data)
        {
            ...
            AppendBranch(tree, 1, data.X);
            AppendBranch(tree, 2, data.Y);
            AppendBranch(tree, 3, data.Z);
        ...
        private class PointBaselineData
        {
            ...
            internal string GetUniqueName(int rowIndex)
            {
                if (UniqueNames == null || rowIndex < 0 || rowIndex >= UniqueNames.Count)
                {
                    return string.Empty;
                }

                string value = UniqueNames[rowIndex];
                return string.IsNullOrWhiteSpace(value) ? string.Empty : value.Trim();
            }

            internal double? GetCoordinate(int columnIndex, int rowIndex)
            {
