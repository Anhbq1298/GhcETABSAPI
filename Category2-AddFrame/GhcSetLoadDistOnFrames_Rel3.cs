...
namespace MGT
{
    public class GhcSetLoadDistOnFrames_Rel3 : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess da)
        {
            ...
            GH_Structure<IGH_Goo> baselineTree = null;
        ...
        private static string GooToTrimmedString(IList branch, int index)
        {
            ...
            {
                return ConvertObjectToTrimmedString(wrapper.Value);
            }

            if (goo.CastTo(out string castText))
            {
                return string.IsNullOrWhiteSpace(castText) ? string.Empty : castText.Trim();
            }

            return ConvertObjectToTrimmedString(goo.ScriptVariable());
        }

        private static string ConvertObjectToTrimmedString(object value)
        {
            if (value == null)
            {
                return string.Empty;
