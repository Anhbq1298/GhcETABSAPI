...
namespace GhcETABSAPI
{
    public class GhcGetLoadDistOnFrames : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_InputParamManager p)
        {
            ...
            // Optional list. If UNCONNECTED or empty → internally treated as null (no filter).
            int idxLP = p.AddTextParameter(
                "loadPattern",
                "loadPattern",
                "Optional load pattern filters. Leave UNCONNECTED or empty to return all (default = null).",
                GH_ParamAccess.list);
            p[idxLP].Optional = true;
        }
        ...
        protected override void SolveInstance(IGH_DataAccess da)
        {
            ...
            try
            {
                // Trim + unique frame names (case-insensitive)
                var trimmed = new List<string>();
                var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                if (frameNames != null)
                {
                    foreach (var nm in frameNames)
                    {
                        if (string.IsNullOrWhiteSpace(nm)) continue;
                        var clean = nm.Trim();
                        if (seen.Add(clean)) trimmed.Add(clean);
                    }
                }

                var rawResult = GetFrameDistributed(sapModel, trimmed);

                // Build trimmed filters only when filters != null
                List<string> trimmedFilters = null;
                if (loadPatternFilters != null)
                {
                    trimmedFilters = new List<string>();
                    var filterSet = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    foreach (var lp in loadPatternFilters)
                    {
                        if (string.IsNullOrWhiteSpace(lp)) continue;
                        var clean = lp.Trim();
                        if (filterSet.Add(clean)) trimmedFilters.Add(clean);
                    }
                }

                bool hasFilters = trimmedFilters != null && trimmedFilters.Count > 0;
                var result = FilterByLoadPattern(rawResult, trimmedFilters);

                var headerTree = BuildHeaderTree();
                var valueTree = BuildValueTree(result);
                ...
                else if (hasFilters)
                {
                    ...
                    if (result.total == 0)
                    {
                        if (rawResult.total > 0)
                        {
                            ...
                            if (result.failCount > 0) status += $" {result.failCount} frame calls failed.";
                        }
        ...
        private static GH_Structure<GH_ObjectWrapper> BuildValueTree(
            (int total, int failCount, List<string> frameName, List<string> loadPat,
             List<int> myType, List<string> cSys, List<int> dir, List<double> rd1, List<double> rd2,
             List<double> dist1, List<double> dist2, List<double> val1, List<double> val2) result)
        {
        ...
        private static object GetValueByColumn(
            (int total, int failCount, List<string> frameName, List<string> loadPat, List<int> myType,
             List<string> cSys, List<int> dir, List<double> rd1, List<double> rd2, List<double> dist1,
             List<double> dist2, List<double> val1, List<double> val2) result,
            int columnIndex, int rowIndex)
        {
            switch (columnIndex)
            {
                case 0: return result.frameName[rowIndex];
                case 1: return result.loadPat[rowIndex];
                case 2: return result.myType[rowIndex];
                case 3: return result.cSys[rowIndex];
                case 4: return result.dir[rowIndex];
                case 5: return result.rd1[rowIndex];
                case 6: return result.rd2[rowIndex];
                case 7: return result.dist1[rowIndex];
                case 8: return result.dist2[rowIndex];
                case 9: return result.val1[rowIndex];
                case 10: return result.val2[rowIndex];
                default: throw new ArgumentOutOfRangeException(nameof(columnIndex));
            }
        }

        /// <summary>
        /// If loadPatternFilters is null or empty → returns the input result unchanged (no filtering).
        /// Otherwise, keeps only rows with loadPattern in the filter set (case-insensitive).
        /// </summary>
        private static (int total, int failCount, List<string> frameName, List<string> loadPat, List<int> myType, List<string> cSys,
            List<int> dir, List<double> rd1, List<double> rd2, List<double> dist1, List<double> dist2, List<double> val1, List<double> val2)
            FilterByLoadPattern(
                (int total, int failCount, List<string> frameName, List<string> loadPat, List<int> myType, List<string> cSys,
                 List<int> dir, List<double> rd1, List<double> rd2, List<double> dist1, List<double> dist2, List<double> val1, List<double> val2) result,
                IReadOnlyCollection<string> loadPatternFilters)
        {
            ...
            var filterSet = new HashSet<string>(loadPatternFilters, comparer);
            ...
            {
                if (!filterSet.Contains(result.loadPat[i])) continue;
            ...
            return (frameNameOut.Count, result.failCount, frameNameOut, loadPatOut, myTypeOut, cSysOut, dirOut,
                rd1Out, rd2Out, dist1Out, dist2Out, val1Out, val2Out);
        }
        ...
        private static string FormatLoadPatternSummary(IReadOnlyList<string> filters)
        {
            ...
            if (filters.Count == 1) return $"load pattern \"{filters[0]}\"";
